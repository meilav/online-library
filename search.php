...
<?php
		...
		echo"<div id='label'> Hasil Pencarian '$k' </div>";

		$query="SELECT * FROM books WHERE judul like '%$k%' or penulis like '%$k%' or penerbit like '%$k%'";
		$exe=mysql_query($query);
		 $jmldata = mysql_num_rows($exe);
		 $jmlhalaman=ceil($jmldata/$batas);

		 if ($jmldata==0){echo "Data Tidak Ditemukan";}
		 else {
		echo"Jumlah Buku Hasil Pencarian: $jmldata
		<div style='color: #104a73;'>
			<center>
			<table id='table' cellpadding='20' cellspacing='20' >";
		$queri= mysql_query("SELECT * FROM books WHERE judul like '%$k%' or penulis like '%$k%' or penerbit like '%$k%' limit $posisi,$batas");
		$no=$posisi;
	...
	echo"
			<tr>
				<td><a href='book.php?id=$kode'><img src='$dir/$cover' title='$judul' alt='$judul' id='img3' /></a></td>
				<td><a href='book.php?id=$kode'><h4>$judul</h4></a>
					<i>$penulis</i><br>
					$penerbit : $kota . $tahun<br>
					<i>$status</i></td>
			</tr>
	";
	if ($no==$jmldata) break;
		...
		echo"
			...
			{
				$previous=$halaman-1;
				echo "<A HREF=\"$file?halaman=1&keyword=$k&sb=$sb\"><< First</A> | 
					<A HREF=\"$file?halaman=$previous&keyword=$k&sb=$sb\">< Previous</A> | ";
			}
			else
			{ 
				//echo "<< First | < Previous | ";
			...
			for($i=$halaman-2;$i<$halaman;$i++)
			{
			  if ($i < 1) 
				  continue;
			  $angka .= "<a href=\"$file?halaman=$i&keyword=$k&sb=$sb\">$i</A> ";
			}
			...
			$angka .= ($halaman+2<$jmlhalaman ? " ...  
					  <a href=\"$file?halaman=$jmlhalaman&keyword=$k&sb=$sb\">$jmlhalaman</A> " : " ");
			...
			{
				$next=$halaman+1;
				echo " | <A HREF=\"$file?halaman=$next&keyword=$k&sb=$sb\">Next ></A> | 
			  <A HREF=\"$file?halaman=$jmlhalaman&keyword=$k&sb=$sb\">Last >></A> ";
			}
			else
			{ 
				//echo " | Next > | Last >>";
